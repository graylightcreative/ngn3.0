<?php
/**
 * Venue Dashboard - Profile Editor
 */
                ...
                if ($entity) {
                    $stmt = $pdo->prepare("
                        UPDATE venues SET 
                            ...
                            address = ?, city = ?, region = ?, capacity = ?,
                            facebook_url = ?, instagram_url = ?, updated_at = NOW()
                        WHERE id = ?
                    ");
                    $stmt->execute([
                        $name, $bio, $email, $phone, $website,
                        $address, $city, $region, $capacity ?: null,
                        $facebook, $instagram, $entity['id']
                    ]);
                } else {
                    $stmt = $pdo->prepare("
                        INSERT INTO venues (user_id, slug, name, bio, email, phone, website, address, city, region, capacity, facebook_url, instagram_url, claimed)
                        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 1)
                    ");
                    $stmt->execute([
                        $user['Id'], $user['Slug'], $name, $bio, $email, $phone, $website,
                        $address, $city, $region, $capacity ?: null, $facebook, $instagram
                    ]);
                }
...
<div class="main-content">
    <header class="page-header">
        ...
        <p class="page-subtitle">Manage your venue profile</p>
    </header>
            ...
            <div class="card">
                ...
                <div class="grid grid-2">
                    ...
                    <div class="form-group">
                        ...
                        <input type="number" name="capacity" class="form-input" min="0"
                               value="<?= htmlspecialchars($entity['capacity'] ?? '') ?>"
                               placeholder="e.g., 500">
                    </div>
                </div>
                ...
                <div class="form-group">
                    ...
                    <textarea name="bio" class="form-textarea" placeholder="Tell people about your venue..."><?= htmlspecialchars($entity['bio'] ?? '') ?></textarea>
                </div>
            ...
            <div class="card">
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="address" class="form-input"
                           value="<?= htmlspecialchars($entity['address'] ?? '') ?>">
                </div>
