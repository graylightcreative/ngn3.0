...
function getFastRisingArtists($artists, $startDate, $endDate, $topN = 10)
{
    $fastRisingArtists = [];
    ...
    foreach ($artists as $artist) {
        ...
        // Append the artist to the fast-rising list if the score is significant
        if ($risingScore > 0) {
            $fastRisingArtists[] = [
                "artist_id" => $artistId,
    ...
    // Sort artists by rising_score (descending)
    usort($fastRisingArtists, function ($a, $b) {
        return $b['rising_score'] - $a['rising_score'];
    });

    // Return the top N artists
    return array_slice($fastRisingArtists, 0, $topN);
}
...
function countMentions($mentionsData, $startTime, $endTime)
{
    return count(array_filter($mentionsData, function ($mention) use ($startTime, $endTime) {
        $timestamp = strtotime($mention['Timestamp']);
        return $timestamp >= $startTime && $timestamp <= $endTime;
    }));
}
