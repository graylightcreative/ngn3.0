...
if ($pdoSmr) {
  try {
    $st = $pdoSmr->query("SELECT COUNT(*) FROM `ngn_smr_2025`.`smr_chart`");
...
// Minimal role checks (presence-based, assuming NGN 2.0 structures)
$roleChecks = [
  'artist' => [ ['Landing tab', $hasArtistsTab], ['DB table', $exists['ngn_2025.artists']], ['API route', _file_has($apiPath,'/artists')] ],
  'label'  => [ ['Landing tab', $hasLabelsTab],  ['DB table', $exists['ngn_2025.labels']],  ['API route', _file_has($apiPath,'/labels')] ],
  'station'=> [ ['Landing tab', $hasStationsTab],['DB table', $exists['ngn_2025.stations']],['API route', _file_has($apiPath,'/stations')] ],
  'venue'  => [ ['DB table', $exists['ngn_2025.venues']], ['API route', _file_has($apiPath,'/venues')] ],
...
?>
    ...
    <main class="max-w-7xl mx-auto px-4 py-6 space-y-6">
      ...
      <section class="rounded-lg border border-gray-200 dark:border-white/10 p-4 bg-white/70 dark:bg-white/5">
        <div class="text-sm font-semibold mb-3">Entity Counts (NGN 2.0)</div>
        <div class="grid grid-cols-2 md:grid-cols-4 gap-3 text-sm">
          <?php
          $entities = ['artists','labels','venues','stations'];
          foreach ($entities as $ent):
            $count = $counts2025[$ent] ?? 0;
            $color = $count > 0 ? 'emerald' : 'rose';
          ?>
          <div class="rounded border border-gray-200 dark:border-white/10 p-3">
            ...
            <div class="w-full bg-gray-200 dark:bg-white/10 rounded h-1.5 mt-1">
              <div class="h-1.5 bg-<?= $color ?>-500 rounded" style="width:100%"></div>
            </div>
            <div class="text-xs text-<?= $color ?>-600 dark:text-<?= $color ?>-400 mt-1"><?= $count > 0 ? 'Data present' : 'No data' ?></div>
          </div>
          <?php endforeach; ?>
        </div>
      </section>
