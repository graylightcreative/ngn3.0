    ...
    <table class="min-w-full text-sm">
      <thead class="text-left text-gray-600 dark:text-gray-300 border-b border-gray-200 dark:border-white/10">
        <tr>
          ...
          <th class="py-2 pr-3">Artists</th>
          <th class="py-2 pr-3">Actions</th>
        </tr>
      </thead>
      <tbody id="labelsTable" class="divide-y divide-gray-100 dark:divide-white/5"></tbody>
    </table>
  </div>
...
async function loadLabels() {
  ...
  let url = api(`/admin/2025/labels?per_page=${perPage}&page=${currentPage}`);
  ...
  const labels = json?.data?.items || [];
  totalLabels = json?.meta?.total || labels.length;
  ...
  labels.forEach(l => {
    ...
    tr.innerHTML = `
      <td class="py-2 pr-3">${l.id}</td>
      <td class="py-2 pr-3 font-medium">${escapeHtml(l.name || '')}</td>
      <td class="py-2 pr-3">${l.website ? `<a href="${escapeHtml(l.website)}" target="_blank" class="text-brand hover:underline">Link</a>` : '-'}</td>
      <td class="py-2 pr-3">${l.claimed ? '✓' : '-'}</td>
      <td class="py-2 pr-3">${l.verified ? '✓' : '-'}</td>
      <td class="py-2 pr-3">${l.artist_count || 0}</td>
      <td class="py-2 pr-3">
        <button class="px-2 h-7 rounded bg-gray-200 dark:bg-white/10 text-xs" onclick="editLabel(${l.id})">Edit</button>
      </td>
    `;
    tbody.appendChild(tr);
  });
  
  document.getElementById('pagerInfo').textContent = `Showing ${labels.length} of ${totalLabels}`;
  document.getElementById('prevPage').disabled = currentPage <= 1;
  document.getElementById('nextPage').disabled = (currentPage * perPage) >= totalLabels;
}

function editLabel(id) {
  fetch(api(`/labels/${id}`), { headers: authHeader() })
    .then(r => r.json())
    .then(json => {
      const l = json?.data || {};
      document.getElementById('editId').value = id;
      document.getElementById('editName').value = l.name || '';
      document.getElementById('editBio').value = l.bio || '';
      document.getElementById('editWebsite').value = l.website || '';
      document.getElementById('editClaimed').checked = !!l.claimed;
      document.getElementById('editVerified').checked = !!l.verified;
...
async function saveLabel() {
  ...
  const { status, json } = await apiCall('PUT', `/admin/labels/${id}`, payload);
  const msg = document.getElementById('editMsg');
  
  if (status === 200) {
    msg.innerHTML = '<span class="text-emerald-500">Saved!</span>';
    setTimeout(() => { closeModal(); loadLabels(); }, 500);
  } else {
...
document.getElementById('searchBtn').addEventListener('click', () => { currentPage = 1; loadLabels(); });
document.getElementById('prevPage').addEventListener('click', () => { if (currentPage > 1) { currentPage--; loadLabels(); } });
document.getElementById('nextPage').addEventListener('click', () => { currentPage++; loadLabels(); });
document.getElementById('refreshBtn').addEventListener('click', loadLabels);
document.getElementById('closeModal').addEventListener('click', closeModal);
document.getElementById('cancelEdit').addEventListener('click', closeModal);
document.getElementById('saveEdit').addEventListener('click', saveLabel);
document.getElementById('searchQ').addEventListener('keypress', e => { if (e.key === 'Enter') { currentPage = 1; loadLabels(); } });
