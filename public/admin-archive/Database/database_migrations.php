...
try{
    ...
    $stmt = $pdo->prepare("CREATE TABLE IF NOT EXISTS migrations (id INT AUTO_INCREMENT PRIMARY KEY, migration VARCHAR(255) NOT NULL, applied_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP)");
    $stmt->execute();

    $stmt = $pdo->prepare("SELECT migration FROM migrations");
    $stmt->execute();
    $appliedMigrations = array_column($stmt->fetchAll(), 'migration');
} catch (\Exception $e){
    $appliedMigrations = [];
}
        ...
        <div class="rounded-lg border border-gray-200 dark:border-white/10 p-4 bg-white/70 dark:bg-white/5">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-right font-medium">
                            ...
                            <?php else: ?>
                                <form method="POST" action="" class="inline">
                                    ...
                                    <button type="submit" name="run_single_migration" class="text-brand hover:text-brand-900 bg-transparent border-none p-0 cursor-pointer">Run</button>
                                </form>
                            <?php endif; ?>
                        </td>
                    </tr>
            ...
            <div class="mt-4">
                <form method="POST" action="">
                    <button type="submit" name="run_migrations" class="inline-flex items-center px-4 py-2 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-brand hover:bg-brand-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-brand-500">Run Pending Migrations</button>
                </form>
            </div>
        </div>
