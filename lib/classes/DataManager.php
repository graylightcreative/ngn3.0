...
class DataManager
{
	...
	{
		$statement = $this->pdo->prepare("SELECT * FROM $table WHERE LOWER($column) = LOWER(:value)");
		$statement->bindValue(':value', $value);
		return $statement->execute() ? $statement->fetch() : false;
	}
	public function readByDB($db, $table, $column, $value)
	{
	...
	public function delete($table, $id)
	{
		...
		$allowedTables = ['spin_data','users','verification_codes','ads', 'albums', 'contacts', 'labels', 'link_locations', 'links', 'orders', 'order_statuses', 'pages', 'posts', 'products', 'product_types', 'shows', 'slide_locations', 'slides', 'songs', 'tours', 'videos']; // Add other valid tables
                ...
                ELSE (
                    ...
                    WHERE 
                        ...
                        (Action = 'label_view' AND
                            (
                                Body LIKE CONCAT('%', (SELECT title FROM users WHERE id = entity_id AND role_id = 7), '%') OR
                                JSON_SEARCH(Tags, 'one', (SELECT title FROM users WHERE id = entity_id AND role_id = 7)) IS NOT NULL
                            )
                        )
                )
	...
	public function fetchAllLabels() {
		return $this->readMany('users', 'role_id', 7);
	}

	public function fetchAllArtists() {
		return $this->readMany('users', 'role_id', 3);
	}

	public function fetchRankingsFromDatabase() {
		return [
			'topArtists' => $this->browse('ranking_items'),
			'topLabels' => $this->browse('ranking_items')
		];
	}
...
//	update 10/02/2024 4:16am est
	...
	{
		$tablesToCheck = [
			'ranking_items' => 'timestamp', // correct in db
			'ranking_history' => 'timestamp', // correct in db
			'ranking_history' => 'timestamp',  // correct in db
			'ranking_history' => 'timestamp',  // correct in db
			'ranking_history' => 'timestamp',  // correct in db
			'posts' => 'published_date', // should be either created, updated, or published_date
			'station_spins' => 'timestamp',  // correct in db
			'post_mentions' => 'timestamp',  // correct in db
			'shows' => 'show_date',  // correct in db
			'videos' => 'timestamp',  // correct in db
			'social_media_posts' => 'timestamp'  // correct in db
		];
		...
		foreach ($tablesToCheck as $table => $timestampColumn) {
			if ($table === 'posts') {
				...
				$sql = "SELECT MAX($timestampColumn) as latest_timestamp 
                  FROM $table 
                  WHERE artist_id = :artistId AND approved = 1";
	...
	public function getLabelIdFromArtistId($artistId)
	{
		...
		if(!$a) return false;
		$l = read('users','label_id',$a['label_id']);
		if($l){
			$labelId = $l['Id'];
		} else {
			return false;
...
// If no chart entries are found, there's no gain to assess
	...
	public function getArticlesWithLabelOrArtistMention(array $articleIds, $labelId)
	{
		...
		$artists = $this->readMany('users', 'label_id', $labelId);
	...
	{
		$artists = readMany('users','label_id',$id);
